...
$rpa = mysql_fetch_array($hasil1);
  ...
  echo "<tr><td colspan=3 align=right><br><b>Total</b>:</td><td colspan=2><br><b>$total_item Item</b></td></tr>
		...
		<a href='media.php?module=home' class='button' style='color:white;background:#a2324c;'>Tambah Barang</a><br/></form><br />
 ...
 $password=$_SESSION[passuser];
    echo "
        ...
        <div class='center_prod_box_big'>            
          <div class='details_big_cari'>
              <div>
          <form method=post action=media.php?module=tambahorder>
		  
		  <table>
		<tr><td>&nbsp;</td><td>&nbsp;<input type=text name=id_sewa value='$id_sewa'/></td></tr>
      
      <tr><td>Tanggal Order</td><td> : <input type=date name=tgl_order  />
     </td></tr>
     ";

// Tampilkan alat-alat yang telah dimasukkan ke keranjang belanja
	$sid = session_id();
	$sql = mysql_query("SELECT *FROM tbl_pesansementara");
 
 $user= $_SESSION[namauser];
 $password=$_SESSION[passuser];


echo "<table border=1 width=620px height=300px cellpadding=3 align=center>
          <tbody>
          <tr bgcolor=#ffffff><th>No</th><th width='120px'>Nama Barang</th><th>Banyak</th><th>Subtotal</th>
          <th width='80px'>Total Jam</th></tr>";  
  ...
  while($r=mysql_fetch_array($sql)){
	...
	$subtotal    = ($r[harga]-$disc) * $r[totaljam]*$r[banyak];
    ...
    echo "<tr bgcolor=#ffffff><td>$no</td>
              <input type=text name=id[$no] value=$r[no_pr_sementara]>
			  <input type=text name=no_alat[$no] value=$r[no_alat] style='width:90%;'>
              <td><input type=text name=nama_brg[$no] style='width:90%;'></td>
				<td><input type=text name=jml[$no] style='width:50px;'></td>
              <td><input type=text name=jam_selesai[$no] style='width:90%;'>
			  </br><font size=2><i>Jam:Menit:Detik</i></font></td>
              <td><input type=text name=subtotal_jam[$no] style='width:30%;'>&nbsp;Jam</td>
              
          </tr>";
		  
    $no++; 
 } 
  echo "<tr><td colspan=6><input type=submit name=submit Value=TAMBAH BARANG size=40></td></tr>
        </tbody></table></form><br />
...
$rpa = mysql_fetch_array($hasil1);
    ...
    echo "
          <p></p>
		  <form method=post action=media.php?module=simpantransaksiorder name='myform'>
		  <table>
      <tr><td>Nama Lengkap</td><td> : <input type=text name=nama value='$rpa[nama]' disabled/></td></tr>
      <tr><td>Alamat</td><td> : <input type=text name=alamat value='$rpa[alamat]' disabled/>
     </td></tr>
	  ...
	  ";
	 echo "<form method='post' id='form-user'>		
		<table>
			<tr>
				<td>Nama Barang</td>
			...
			<tr>
				<td>Banyak</td>
				<td><input type='number' name='banyak' style='width:50px;'></td>
			</tr>
			<tr>
				<td></td>
				<td><a class='tombol-simpan button'>Simpan</a></td>
			</tr>
		</table>
	</form>
...
// simpan data sewa 
mysql_query("INSERT INTO tbl_permintaan(tgl_pr, nip,status) VALUES('$tgl_skrg','$nip','Belum Disetujui')");
...
<td>Cilegon, $tgl_skrg2</td>
</tr>
</table><p></p>
<p><b>*Simpan dan catat nomor permintaan ini sebagai bukti permintaan barang</b></p> 
<a href='media.php?module=home' class='button' style='color:white;background:#a2324c;'>Kembali Ke Beranda</a></br>
...
function isi_keranjang(){
	$isikeranjang = array();
	$sid = session_id();
	$sql = mysql_query("SELECT * FROM tbl_pesansementara where id_session='$sid'");
...
// simpan data sewa 
mysql_query("INSERT INTO tbl_pesanh(tgl_order, nip,status) VALUES('$tgl_skrg','$nip','Belum Disetujui')");
$no_order=mysql_insert_id();

// simpan data detail sewa  
for ($i = 0; $i < $jml; $i++)
{
  mysql_query("INSERT INTO tbl_pesand 
               VALUES('$no_order',
			   '{$isikeranjang[$i]['nama_barang']}',
				'{$isikeranjang[$i]['banyak']}')");
				}
for ($i = 0; $i < $jml; $i++) {
  mysql_query("DELETE FROM tbl_pesansementara
	  	         WHERE no_order_sementara = {$isikeranjang[$i]['no_order_sementara']}");
}
 $array_bulan = array(1=>"I","II","III", "IV", "V","VI","VII","VIII","IX","X", "XI","XII");
 $bln_skrg = $array_bulan[date('n')];
//$bln_skrg = date("m");
$thn_skrg = date ("Y");
      ...
      <table>
	  <tr><td>No. Order </td><td> : <b>$no_order / CHC / FPNMB / $bln_skrg / $thn_skrg</b> </td></tr>
      <tr><td>Nama Lengkap   </td><td> : <b>$r[nama]</b> </td></tr>
      <tr><td>Jabatan </td><td> :<b> $r[jabatan]</b> </td></tr>
      </table><hr /><br />";

      $daftaralat=mysql_query("SELECT * FROM tbl_pesand 
...
$no=1;
while ($d=mysql_fetch_array($daftaralat)){
	$subtotalitemtabel       = $d[jumlah];
   $subtotalitem       = $subtotalitem + $d[jumlah]; 
    $total_item    = $subtotalitem;

   echo "<tr><td>$no</td><td>$d[nama_barang]</td><td align=center>$d[jumlah]</td>
                             <td align=right>$subtotalitemtabel</td></tr>";
